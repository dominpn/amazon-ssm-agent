...
func (suite *TelemetryTestSuite) TestInitialize() {
	...
	assert.Equal(suite.T(), filewatcherbasedipc.ModeMaster, fakeChannel.GetMode())
}
...
func (suite *TelemetryTestSuite) Test_emitLog() {
	...
	receiveIpc := channelmock.NewFakeChannel(suite.mockContext.Log(), filewatcherbasedipc.ModeWorker, suite.mockContext.ChannelName())
